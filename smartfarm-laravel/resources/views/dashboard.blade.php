<x-app-layout>
    ...
    <div class="space-y-6 animate-fade-in-up">
        <!-- Welcome Section -->
        <div class="relative overflow-hidden rounded-3xl p-8 text-white shadow-sm">
            <!-- Background Image -->
            <div class="absolute inset-0 pointer-events-none z-0">
                <img src="{{ asset('4096x1645.webp') }}" class="w-full h-full object-cover object-[center_40%] scale-110" />
                <div class="absolute inset-0 bg-linear-to-r from-black/85 via-black/35 via-50% to-transparent to-75%"></div>
            </div>
            ...
            <div class="relative z-10 max-w-2xl">
                ...
                <div class="mt-6 flex flex-wrap gap-3">
                    <a href="{{ route('predictions.create') }}" class="inline-flex items-center gap-2 rounded-full bg-emerald-600 px-6 py-2.5 text-sm font-bold text-white transition-all hover:bg-emerald-700 hover:-translate-y-0.5 active:translate-y-0 shadow-sm">
        ...
        <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="group rounded-3xl border border-slate-200 bg-white p-6 transition-all duration-300 hover:border-emerald-200">
                <div class="flex items-center justify-between">
                    ...
                    @if($totalPredictions > 0)
                        <span class="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full uppercase tracking-wider">Riwayat</span>
                    @endif
                </div>
                <div class="mt-4">
                    ...
                    <p class="mt-1 text-xs font-semibold text-slate-400">Total Eksperimen Lahan</p>
                </div>
            </div>
            ...
            <div class="group rounded-3xl border border-slate-200 bg-white p-6 transition-all duration-300 hover:border-emerald-200 min-w-0">
                ...
                <div class="mt-4">
                    ...
                    <p class="mt-1 text-xs font-semibold text-slate-400">Rekomendasi Tanaman Terakhir</p>
                </div>
            </div>
            ...
            <div class="group rounded-3xl border border-slate-200 bg-white p-6 transition-all duration-300 hover:border-emerald-200 sm:col-span-2 lg:col-span-1 min-w-0">
                ...
                <div class="mt-4">
                    <p class="font-outfit text-3xl font-black text-slate-900 sm:text-4xl truncate">
                        ...
                        @endphp
                        {{ $latestLand ? (str_contains($latestLand, ' / ') ? explode(' / ', $latestLand)[0] : $latestLand) : '-' }}
                    </p>
                    <p class="mt-1 text-xs font-semibold text-slate-400">Klasifikasi Lahan Terakhir</p>
                </div>
            </div>
        </div>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @forelse($latestPredictions as $prediction)
                                <tr class="group hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="text-xs font-semibold text-slate-500">{{ str_contains($prediction->land_type, ' / ') ? str_replace(' / ', ' (', $prediction->land_type) . ')' : $prediction->land_type }}</span>
                                    </td>
